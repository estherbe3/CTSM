...
! Excess ice Soil tiling varibales
    ...
    real(r8)           :: om_frac_t2                       ! organic matter fraction for tile 2
...
! remove roots below bedrock layer
 ...
 !read in soil thermal properties for Excess ice tiling
    if (use_excess_ice_tiles) then 
      write(iulog,*) 'Excessice tiling Soil'
      allocate(sand3d_t2(begg:endg,nlevsoifl))
      allocate(clay3d_t2(begg:endg,nlevsoifl))
      allocate(organic3d_t2(begg:endg,nlevsoifl))
      call ncd_io(ncid=ncid, varname='ORGANIC_TILE2', flag='read', data=organic3d_t2, dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
         write(iulog,*) (' Warning: Organic Tile2 not on surfdata file') 
        ! call organicrd(organic3d_t2)
       end if
     write(iulog,*) 'allocate data'
    
      call ncd_io(ncid=ncid, varname='PCT_SAND_TILE2', flag='read', data=sand3d_t2, dim1name=grlnd, readvar=readvar)
     
          if (.not. readvar) then
            write(iulog,*) (' Warning: PCT_SAND_TILE2 NOT on surfdata file') 
      ...
      call ncd_io(ncid=ncid, varname='PCT_CLAY_TILE2', flag='read', data=clay3d_t2, dim1name=grlnd, readvar=readvar)
          if (.not. readvar) then
            write(iulog,*) (' Warning: PCT_CLAY_TILE2 NOT on surfdata file')
          ...
          do lev = 1,nlevgrnd
             ...
             if (lun%itype(l) /= istdlak) then  ! soil columns of both urban and non-urban types
                ...
                soilstate_inst%watopt_col(c,lev) = soilstate_inst%watsat_col(c,lev) * &
                     ...
                     om_frac_t2 = min(params_inst%om_frac_sf*organic3d(g,nlevsoifl)/organic_max, 1._r8)      
