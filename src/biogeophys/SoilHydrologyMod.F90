...
!#4
   ...
   subroutine PerchedLateralFlow(bounds, num_hydrologyc, filter_hydrologyc, &
     ...
     associate(                                                            & 
         ...
         c = filter_hydrologyc(fc)
            ...
            if (frost_table(c) > zwt_perched(c)) then
               ...
               if (lun%itype(col%landunit(c)) == istsoil .and.lun%ncolumns(l) == 2 .and. exice_tile_mask(g) == 1) then  
                 ...
                 ! if (c==c_src) then
                   !qflx_drain_perched(c_dst)=qflx_drain_perched(c_dst)-abs(1.e3_r8*(transmis*dl*head_gradient/A_drc)) 
                   !qflx_drain_perched(c_src)=qflx_drain_perched(c_src)+abs(1.e3_r8*(transmis*dl*head_gradient/A_src)) 
                  !endif
                ...
                ! remove drainage from perched saturated layers
                    ...
                    do k = k_perch(c_src), k_frost(c_src)-1
                        ...
                        write(iulog, *) "H2O_liq", h2osoi_liq(c_src,k), "c", c, "k", k, drainage_layer
                        h2osoi_liq(c_src,k) = h2osoi_liq(c_src,k) - drainage_layer
                        write(iulog, *) "H2O_liq updated", h2osoi_liq(c_src,k)
                  enddo
                  ...
                  write(iulog, *) "Drainage", qflx_drain_perched
                    ...
                    if (frost_table(c_dst) > zwt_perched(c_dst)) then
                        ...
                        * sin(col%topo_slope(c_dst) * (rpi/180._r8))
                        wtsub = 0._r8
                        q_perch = 0._r8
                        q_perch_out = 0._r8
                        do k = k_perch(c_dst), k_frost(c_dst)-1
      ...
      write(iulog, *) "drainage end Perch", c, qflx_drain_perched
